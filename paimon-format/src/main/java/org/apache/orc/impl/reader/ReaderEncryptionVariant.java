...
public class ReaderEncryptionVariant implements EncryptionVariant {
    ...
    ReaderEncryptionVariant(
        ...
        this.column =
                proto == null || !proto.hasRoot() ? schema : schema.findSubtype(proto.getRoot());
        ...
        if (proto != null && proto.hasEncryptedKey()) {
            ...
            footerKey =
                    getCachedKey(cache, key.getAlgorithm(), proto.getEncryptedKey().toByteArray());
            key.addVariant(this);
     ...
     */
    private static LocalKey getCachedKey(
            Map<BytesWritable, LocalKey> cache, EncryptionAlgorithm algorithm, byte[] encrypted) {
            ...
            switch (this.key.getState()) {
                case UNTRIED:
                    if (plaintextDataKey != null) {
                        result = new SecretKeySpec(plaintextDataKey, "AES");
    ...
    public List<StripeStatistics> getStripeStatistics(
            boolean[] columns, InStream.StreamOptions compression, ReaderImpl reader)
            throws IOException {
        ...
        for (OrcProto.Stream stream : stripeStats) {
            ...
            if (kind == OrcProto.Stream.Kind.STRIPE_STATISTICS
                ...
                InStream.StreamOptions options =
                        new InStream.StreamOptions(compression)
                ...
                OrcProto.ColumnarStripeStatistics stat =
                        OrcProto.ColumnarStripeStatistics.parseFrom(
                                InStream.createCodedInputStream(
                                        InStream.create(
                                                stream, tailBuffer, offset, length, options)));
                for (int s = 0; s < result.length; ++s) {
