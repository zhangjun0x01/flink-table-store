...
public class ReaderImpl implements Reader {
    ...
    public List<StripeStatistics> getVariantStripeStatistics(EncryptionVariant variant)
        ...
        if (variant == null) {
                ...
                try (CompressionCodec codec = OrcCodecPool.getCodec(compressionKind)) {
                    ...
                    stripeStatistics =
                            deserializeStripeStats(
                                    tail.getTailBuffer(),
                                    tail.getMetadataOffset(),
                                    tail.getMetadataSize(),
                                    options);
                }
            }
            ...
            try (CompressionCodec codec = OrcCodecPool.getCodec(compressionKind)) {
                ...
                return ((ReaderEncryptionVariant) variant)
                        .getStripeStatistics(null, compression, this);
            }
        }
    }
                    ...
                    if (variant != null) {
                        try {
                            ...
                            ColumnStatistics[] overrides =
                                    decryptFileStats(variant, compression, tail.getFooter());
                            for (int sub = 0; sub < overrides.length; ++sub) {
                                result[base + sub] = overrides[sub];
                            }
                        } catch (IOException e) {
                            throw new RuntimeException(
                                    "Can't decrypt file stats for "
                                            + path
                                            + " with "
                                            + variant.getKeyDescription());
                        }
                    }
                }
            }
    ...
    private ColumnStatistics[] decryptFileStats(
            ReaderEncryptionVariant encryption,
        ...
        if (key == null) {
            ...
            CryptoUtils.modifyIvForStream(
                            encryption.getRoot().getId(),
                            OrcProto.Stream.Kind.FILE_STATISTICS,
                            footer.getStripesCount() + 1)
                    .accept(iv);
            InStream.StreamOptions options =
                    new InStream.StreamOptions(compression).withEncryption(algorithm, key, iv);
            InStream in = InStream.create("encrypted file stats", buffer, 0, bytes.length, options);
    ...
    public ColumnStatistics[] deserializeStats(
            TypeDescription schema, List<OrcProto.ColumnStatistics> fileStats) {
    ...
    /**
     * Ensure this is an ORC file to prevent users from trying to read text files or RC files as ORC
     * files.
     *
     ...
     */
    protected static void ensureOrcFooter(
            FSDataInputStream in, Path path, int psLen, ByteBuffer buffer) throws IOException {
        int magicLength = OrcFile.MAGIC.length();
        int fullLength = magicLength + 1;
        if (psLen < fullLength || buffer.remaining() < fullLength) {
            throw new FileFormatException(
                    "Malformed ORC file " + path + ". Invalid postscript length " + psLen);
        }
        ...
        if (!Text.decode(array, offset, magicLength).equals(OrcFile.MAGIC)) {
            ...
            if (!Text.decode(header, 0, magicLength).equals(OrcFile.MAGIC)) {
                throw new FileFormatException(
                        "Malformed ORC file " + path + ". Invalid postscript.");
            }
        }
    }
     ...
     * @param psLen the postscript length
     * @param buffer the tail of the file
     * @deprecated Use {@link ReaderImpl#ensureOrcFooter(FSDataInputStream, Path, int, ByteBuffer)}
     *     instead.
     */
    protected static void ensureOrcFooter(ByteBuffer buffer, int psLen) throws IOException {
        ...
        if (!Text.decode(array, offset, magicLength).equals(OrcFile.MAGIC)) {
            ...
            if (!Text.decode(buffer.array(), 0, magicLength).equals(OrcFile.MAGIC)) {
                throw new FileFormatException(
                        "Malformed ORC file. Invalid postscript length " + psLen);
            }
        }
    }
    ...
    /**
     * Check to see if this ORC file is from a future version and if so, warn the user that we may
     * not be able to read all of the column encodings.
     *
     * @param path the data source path for error messages
     * @param postscript the parsed postscript
     */
    protected static void checkOrcVersion(Path path, OrcProto.PostScript postscript)
            throws IOException {
        List<Integer> version = postscript.getVersionList();
        if (getFileVersion(version) == OrcFile.Version.FUTURE) {
            throw new IOException(
                    path
                            + " was written by a future ORC version "
                            + versionString(version)
                            + ". This file is not readable by this version of ORC.\n"
                            + "Postscript: "
                            + TextFormat.shortDebugString(postscript));
        }
    }
     ...
     * @param bytes
     */
    public ReaderImpl(Path path, OrcFile.ReaderOptions options, byte[] plaintextDataKey)
            throws IOException {
        this.path = path;
        ...
        if (fileMetadata != null) {
            ...
            this.encryption =
                    new ReaderEncryption(
                            footer,
                            schema,
                            tail.getStripeStatisticsOffset(),
                            tail.getTailBuffer(),
                            stripes,
                            options.getKeyProvider(),
                            conf,
                            plaintextDataKey);
            this.softwareVersion =
                    OrcUtils.getSoftwareVersion(footer.getWriter(), footer.getSoftwareVersion());
        }
    ...
    public static OrcProto.Metadata extractMetadata(
            ByteBuffer bb, int metadataAbsPos, int metadataSize, InStream.StreamOptions options)
            throws IOException {
        bb.position(metadataAbsPos);
        bb.limit(metadataAbsPos + metadataSize);
        return OrcProto.Metadata.parseFrom(
                InStream.createCodedInputStream(
                        InStream.create(
                                "metadata", new BufferChunk(bb, 0), 0, metadataSize, options)));
    }

    private static OrcProto.PostScript extractPostScript(
            BufferChunk buffer, Path path, int psLen, long psOffset) throws IOException {
        CodedInputStream in =
                InStream.createCodedInputStream(InStream.create("ps", buffer, psOffset, psLen));
    ...
    /**
     * @deprecated Use {@link ReaderImpl#extractFileTail(FileSystem, Path, long)} instead. This is
    ...
    /**
     * Read compression block size from the postscript if it is set; otherwise, use the same 256k
     * default the C++ implementation uses.
     */
    ...
    public static OrcTail extractFileTail(ByteBuffer buffer, long fileLen, long modificationTime)
        ...
        buffer.clear();
        return new OrcTail(
                fileTailBuilder.build(), new BufferChunk(buffer.slice(), 0), modificationTime);
    }

    protected OrcTail extractFileTail(FileSystem fs, Path path, long maxFileLength)
            throws IOException {
        BufferChunk buffer;
        OrcProto.PostScript ps;
        OrcProto.FileTail.Builder fileTailBuilder = OrcProto.FileTail.newBuilder();
        ...
        try {
            ...
            if (size == 0) {
                ...
                throw new FileFormatException(
                        "Not a valid ORC file " + path + " (maxFileLength= " + maxFileLength + ")");
            }
            ...
            throw thr instanceof IOException
                    ? (IOException) thr
    ...
    static long getRawDataSizeFromColIndices(
            boolean[] include, TypeDescription schema, List<OrcProto.ColumnStatistics> stats) {
        long result = 0;
        for (int c = schema.getId(); c <= schema.getMaximumId(); ++c) {
            if (include[c]) {
    ...
    private static long getRawDataSizeOfColumn(
            TypeDescription column, List<OrcProto.ColumnStatistics> stats) {
    ...
    private static List<OrcProto.StripeStatistics> deserializeStripeStats(
            BufferChunk tailBuffer, long offset, int length, InStream.StreamOptions options)
            throws IOException {
        InStream stream = InStream.create("stripe stats", tailBuffer, offset, length, options);
        OrcProto.Metadata meta =
                OrcProto.Metadata.parseFrom(InStream.createCodedInputStream(stream));
                                ...
                                for (int sub = c; sub <= variantType.getMaximumId(); ++sub) {
                                    if (included == null || included[sub]) {
                                        for (int s = 0; s < colStats.size(); ++s) {
                                            StripeStatisticsImpl resultElem =
                                                    (StripeStatisticsImpl) result.get(s);
                                            resultElem.updateColumn(
                                                    sub,
                                                    colStats.get(s)
                                                            .getColumn(sub - variantType.getId()));
                                        }
                                    }
                                }
    ...
    /**
     * Take the file from the reader. This allows the first RecordReader to use the same file, but
     * additional RecordReaders will open new handles.
     *
